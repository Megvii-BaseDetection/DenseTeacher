...
class SemiRunner(DefaultRunner):
    def __init__(self, cfg, build_model):
        ...
        # self.ema_checkpointer = DefaultCheckpointer(
    ...
    def build_hooks(self):
        ...
        def test_and_save_ema_results():
            ...
            self._last_eval_results = self.test(self.cfg, self.ema_model.model)
    ...
    def resume_or_load(self, resume=True):
        ...
        if not getattr(self.cfg.TRAINER.EMA, 'FAKE', False):
            ...
            if resume:
                ...
                # ema_checkpoint = self.ema_checkpointer.get_checkpoint_file().replace('.pth', '_ema.pth')
                # self.start_iter = (self.ema_checkpointer.resume_or_load(
                #     ema_checkpoint, resume=False).get("iteration", -1) + 1)
            else:
                self.start_iter = (self.checkpointer.resume_or_load(
                    self.cfg.MODEL.WEIGHTS, resume=False).get("iteration", -1) + 1)
                # self.start_iter = (self.ema_checkpointer.resume_or_load(
                #     self.cfg.MODEL.WEIGHTS[1], resume=False).get("iteration", -1) + 1)
